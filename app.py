# ==============================================================================
# MODIFIED BY: Alexandro Alves Madi
# DATE: May 2026
# INSTITUTION: UFSCar / NIT Materiais
#
# MODIFICATIONS:
# 1. Fixed os.walk recursive zip extraction bug for nested directories.
# 2. Updated deprecated Flask 'attachment_filename' to 'download_name'.
# 3. Implemented Anti-Cache headers for dynamic UI (Lentille) loading.
# 4. Implemented Auto-Discovery for orphaned data folders physically in ./DATA/.
# 5. Removed hardcoded absolute paths (/home/p2n/...), making the code agnostic.
# ==============================================================================
...
# Descobre automaticamente a pasta onde o app.py está rodando
BASE_DIR = os.path.abspath(os.path.dirname(__file__))

# --- CORREÇÃO DEFINITIVA: ANTI-CACHE NUCLEAR PARA A LENTILLE ---
# Desativa o cache nativo do Flask para arquivos estáticos
app.config['SEND_FILE_MAX_AGE_DEFAULT'] = 0

@app.after_request
def add_header(response):
    """
    Previne que o navegador faça cache agressivo destruindo
    completamente os validadores de arquivo (ETag e Last-Modified).
    Garante que a UI reflita pesquisas recém concluídas.
    """
...
def get_requests():
    ...
    # --- AUTO-DISCOVERY DE PASTAS ---
    # Vasculha a pasta DATA fisicamente e força os projetos órfãos a aparecerem na interface
    if os.path.exists('./DATA/'):
        pastas_data = [d for d in os.listdir('./DATA/') if os.path.isdir(os.path.join('./DATA/', d))]
        for pasta in pastas_data:
            if pasta not in dex['done'] and pasta not in dex['in_progress']:
                dex['done'].append(pasta)
    # ---------------------------------------------
...
def update_one_request_interface(p2n_dir):
    print(p2n_dir)
    os.chdir(BASE_DIR)
    os.chdir(os.path.join(BASE_DIR, "Patent2Net"))
...
# --- RECURSIVIDADE E COMPRESSAO CORRIGIDAS MANTIDAS INTACTAS ---
@app.route('/downloadDat', methods=['GET','POST'])
def requestData_zip():
    data = io.BytesIO()
    pasta_base = './DATA/'
    # O uso de ZIP_STORED evita arquivos corrompidos e reduz consumo de memoria
    with zipfile.ZipFile(data, mode='w', compression=zipfile.ZIP_STORED) as z:
        for root, dirs, files in os.walk(pasta_base):
            for file in files:
                caminho_completo = os.path.join(root, file)
                # Cria o caminho relativo para manter as pastas organizadas dentro do ZIP
                caminho_relativo = os.path.relpath(caminho_completo, pasta_base)
                z.write(caminho_completo, arcname=caminho_relativo)
    data.seek(0)
    return send_file(
        data,
